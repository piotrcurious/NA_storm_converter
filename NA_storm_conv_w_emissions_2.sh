...
convert_name() {
    local name=$1
    local year=$2
    local storm_number=$3
    local list_number=$((year % 6))
    [ $list_number -eq 0 ] && list_number=6

    local list_var
    local supplemental=false

    if [ $storm_number -le 21 ]; then
        list_var="list${list_number}[${name}]"
    else
    ...
    if [ -n "$new_name_and_emissions" ]; then
        ...
        if [ "$emissions" = "N/A" ]; then
            echo "$name converted to $new_name (${supplemental:+Supplemental }List ${supplemental:-$list_number})"
        ...
        echo "No conversion found for $name in ${supplemental:+Supplemental }List ${supplemental:-$list_number}"
    fi
}
...
while read -p "> " name; do
    ...
    read -p "> " year
    convert_name "$name" "$year" $storm_count
    echo "----------------------------------------"
    ((storm_count++))
done
